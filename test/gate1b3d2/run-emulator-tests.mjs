// Starts a local Firestore emulator scoped to test/gate1b3d2 (own firebase.json/.firebaserc,
// demo project id, dedicated port 8184 — distinct from every prior gate's port), runs the
// Gate 1B.3-D2 runtime-bridge/reopen-guard tests against it (loaded with the frozen, UNCHANGED
// Gate 1B.3-C1X Rules candidate — D2 never modifies Rules), then shuts down ONLY the exact
...
function copyCandidateRules() {
  ...
  console.log(`[gate1b3d2] copied firestore.rules.production-candidate -> test/gate1b3d2/firestore.rules (sha256 ${hash})`);
}
...
async function main() {
  copyCandidateRules();
  console.log(`[gate1b3d2] starting Firestore emulator (emulators:start, NOT deploy) in ${here} on port ${port}...`);
  ...
  try {
    ...
    console.error("[gate1b3d2] emulator startup failed:", e.message);
  } finally {
    console.log("[gate1b3d2] shutting down emulator (targeted kill only)...");
    try { emulator.kill(); } catch { /* ignore */ }
    await killByCommandLineMatch("demo-hcma2-gate1b3d2");

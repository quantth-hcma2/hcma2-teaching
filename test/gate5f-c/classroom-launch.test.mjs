...
test("error mapping: every documented Gate 5C code maps to a Vietnamese, non-technical message", () => {
  for (const code of ["UNAUTHENTICATED", "FORBIDDEN", "BAD_SESSION", "SESSION_NOT_FOUND", "SESSION_DELETED", "BAD_IDEMPOTENCY_KEY", "INVALID_EPOCH", "NETWORK", "INVALID_BOOTSTRAP_URL", "POPUP_BLOCKED"]) {
...
// -----------------------------------------------------------------------------------
// GATE 5F.D2.G13B — INVALID_EPOCH (new Gate 5F.D2.G12B backend code: startProjection() rejects
// when the Knowledge session's grantEpoch is missing/invalid at the moment of the in-transaction
// reread, before any write is staged — verified against projections.mjs, never ambiguous).
// -----------------------------------------------------------------------------------
test("error mapping: INVALID_EPOCH gets a specific, non-generic message that never mentions 'epoch' or internal state", () => {
  const msg = classroomErrorMessage("INVALID_EPOCH");
  const DEFAULT = classroomErrorMessage("SOME_FUTURE_CODE"); // the generic fallback, for contrast
  assert.notEqual(msg, DEFAULT, "INVALID_EPOCH must not silently fall through to the generic default message");
  assert.doesNotMatch(msg, /epoch/i, "message must not leak the internal grantEpoch concept to the lecturer");
  assert.match(msg, /tải lại trang|thử lại/i, "message must direct the lecturer to refresh/retry");
});

test("server error code INVALID_EPOCH passes through Start's failure path to the mapped message (definitive rejection, not ambiguous — no recheckStatus)", async () => {
  const { controller } = fakeController({
    fetchImpl: async () => ({ ok: false, json: async () => ({ error: { code: "INVALID_EPOCH", message: "Knowledge session has an invalid grantEpoch value." } }) }),
  });
  const result = await controller.start(SYNTHETIC_SESSION);
  assert.equal(result.ok, false);
  assert.equal(result.code, "INVALID_EPOCH");
  assert.equal(result.message, classroomErrorMessage("INVALID_EPOCH"));
  // Thrown entirely within the transaction's read phase, before any write is staged (verified
  // against projections.mjs) — exactly as definitive as SESSION_DELETED/FORBIDDEN/etc., so no
  // recheckStatus flag (that is reserved for genuinely ambiguous NETWORK failures only).
  assert.ok(!result.recheckStatus, "INVALID_EPOCH is a definitive rejection — must not trigger a status recheck");
});
